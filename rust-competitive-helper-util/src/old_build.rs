...
mod build_use_tree_tests {
    use crate::old_build::build_use_tree_full_line;
...
pub fn add_rerun_if_changed_instructions(libraries: &[String]) {
    let add = |file: &str| {
...
fn build_main_fun<F: FileExplorer>(file_explorer: &F) -> String {
    ...
    let task = crate::parse_task(file_explorer).expect("Can't parse task json");
...
pub(crate) fn build_several_libraries_impl<F: FileExplorer>(
    ...
    let mut code = Vec::new();
    if let Some(task) = crate::parse_task(file_explorer) {
